...
# In-memory report storage (production: use S3 / filesystem)
...
async def generate_report(
    ...
    if fmt == "csv":
        ...
        filename = f"kovirx_report_{report_type}_{report_id.hex[:8]}.csv"
    else:
        content = _generate_pdf_content(alerts, predictions, device_count, period_start, period_end)
        filename = f"kovirx_report_{report_type}_{report_id.hex[:8]}.pdf"

    # Store report metadata + content
    ...
    logger.info("Report generated: %s (%s)", filename, report_type)
...
def get_report_content(report_id: str) -> dict | None:
...
def list_reports() -> list[dict]:
    ...
    return [
        {"id": k, "filename": v["filename"]}
        for k, v in _report_store.items()
    ]
