...
class NetworkFlow(Base, UUIDMixin, TimestampMixin):
    ...
    start_time: Mapped[None] = mapped_column(DateTime(timezone=True), nullable=True, index=True)
    end_time: Mapped[None] = mapped_column(DateTime(timezone=True), nullable=True, index=True)
